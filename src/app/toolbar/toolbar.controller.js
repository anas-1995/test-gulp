(function () {
    ...
    /** @ngInject */
    function ToolbarController($rootScope, $q, $state, $scope, $timeout, $mdSidenav, $translate, $mdToast, AuthenticationService, msNavigationService, msApi) {
        var vm = this;
        ...
        function setBusiness(item) {
            AuthenticationService.setSelectedBusiness(item)
            $scope.selectedBusiness = { "id": item.id, "name": item.name }
        ...
        function init() {
            ...
            if ($scope.role != 'ADMIN') {
                var params = { user_id: $scope.userId }

                msApi.request('businesses.businesses@get', params,
                    function (res) {
                        console.log(res);
                        $scope.businesses = res.data;
                        $scope.selectedBusiness = AuthenticationService.getSelectedBusiness()
                        if ($scope.selectedBusiness == null) {
                            setBusiness($scope.businesses[0])
                        }
